...
class DiffusionModule(nn.Module):
    ...
    def q_sample(self, x0, t, noise=None):
        ...
        xt = torch.sqrt(alphas_prod_t)*x0+ noise*torch.sqrt(1-alphas_prod_t)
    ...
    def p_sample(self, xt, t):
        ...
        x_t_prev = (1/extract(self.var_scheduler.alphas, t, xt).sqrt())*(xt - eps_factor*eps_theta)
          # Add noise if t > 0
        if (t == 0).all():
            pass # no noise at t=0
        else:
            # Typically beta_t is used to compute the variance
            beta_t = extract(self.var_scheduler.betas, t, xt)
            noise = torch.randn_like(xt)
            x_t_prev= x_t_prev + torch.sqrt(beta_t) * noise
        #######################
        return x_t_prev
    ...
    def p_sample_loop(self, shape):
        ...
        x_t = torch.randn(shape).to(self.device)  # Start from pure noise
    ...
    def ddim_p_sample(self, xt, t, t_prev, eta=0.0):
        ...
        alpha_prod_t_prev = (
            ...
            if t_prev >= 0 else torch.ones_like(alpha_prod_t)
        )
        ...
        # Compute DDIM sigma (noise scale)
        sigma = (
            eta * torch.sqrt((1 - alpha_prod_t_prev) / (1 - alpha_prod_t)) *
            torch.sqrt(1 - (alpha_prod_t / alpha_prod_t_prev))
        )

        # Compute final x_{t-1}
        eps_scale = torch.sqrt(1 - alpha_prod_t_prev - sigma**2)
        noise = torch.randn_like(xt) if t > 0 else 0.0
        x_t_prev = (
            torch.sqrt(alpha_prod_t_prev) * x0_pred +
            eps_scale * eps_theta +
            sigma * noise
        )
    ...
    def ddim_p_sample_loop(self, shape, num_inference_timesteps=50, eta=0.0):
        ...
        # Create reverse-ordered inference timesteps
        timesteps = (
        ...
        prev_timesteps = torch.cat([timesteps[1:], torch.tensor([-1], device=self.device)])

        # Start from pure Gaussian noise
        xt = torch.randn(shape, device=self.device)
        # print(f'mean of xt: {xt.mean().item()}, std of xt: {xt.std().item()}')
    ...
    def compute_loss(self, x0):
        ...
        noise = torch.randn_like(x0)
        xts = self.q_sample(x0,t, noise=noise)
        eps_thetas = self.network(xts,t)
        loss = torch.nn.functional.mse_loss(eps_thetas, noise)
